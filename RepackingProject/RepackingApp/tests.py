...
from RepackingApp.models import MeetingModel, TypeMeetingModel
from RepackingApp.services.records import request_recordings, parse_xml_recordings, \
    ...
    get_type_recordings, \
    upload_from_source, get_recordings, get_recordings_to_dict, get_type_recordings_to_dict, update_recordings_fields, \
...
class RepackingServiceTests(TestCase):
    ...
    def test_parse_xml_recordings(self):
        ...
        self.assertEqual(type(recordings[0][1]), MeetingModel)
        self.assertEqual(type(type_recordings[0]), TypeMeetingModel)
    ...
    def test_parse_xml_type_recording(self):
        ...
        self.assertEqual(type(res), TypeMeetingModel)
    ...
    def test_parse_xml_recording(self):
        ...
        self.assertEqual(type(res), MeetingModel)
    ...
    def test_upload_recordings(self):
        ...
        tm = TypeMeetingModel.objects.all()
        self.assertEqual(len(tm), 5)

        tm = MeetingModel.objects.all()
    ...
    def test_upload_from_source(self):
        ...
        recordings_len = MeetingModel.objects.count()
    ...
    def test_get_recordings_by_name(self):
        data = parse_xml_recordings(self.content)
        upload_recordings_to_db(data)

        name = "Информационные технологии в задачах филологии и компьютерной лингвистики"
        recordings = get_recordings(type_meeting__name=name)

        self.assertEqual(len(recordings), 2)
        self.assertEqual(recordings[0].type_meeting.name, name)
    ...
    def test_get_recordings_by_id(self):
        ...
        pk = 1
        recordings = get_recordings(type_meeting__id=pk)

        self.assertEqual(len(recordings), 2)
        self.assertEqual(recordings[0].type_meeting.id, pk)

    def test_get_recordings_to_dict_by_id(self):
        data = parse_xml_recordings(self.content)
        upload_recordings_to_db(data)
    ...
    def test_update_recordings_fields(self):
        ...
        recordings_filter = list(filter(lambda x: x.url != '', MeetingModel.objects.all()))
        ...
        test_recordings = [MeetingModel(**item) for item in test_recordings_dict]
